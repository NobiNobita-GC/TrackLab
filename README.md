...
`HotPlateModuleViewModel` 已提供工艺开始与完成操作；冷板详情目前只展示模块信息，Load Port 和机械手使用通用详情弹窗。热板、冷板页面的温度仍为 `-- °C` 占位文本，尚未绑定实时温度。模块实体已预留 `SetTemperature`、`ActualTemperature`、`HasCarrier` 等演示属性，同样尚未接入界面。
